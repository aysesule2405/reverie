...
export default function EditMood() {
  ...
  const addTag = () => {
    ...
    if (t && !moodTags.includes(t)) setMoodTags((p) => [...p, t]);
    setTagInput('');
  };
  ...
  const addSong = () => {
    ...
    if (s && !songLinks.includes(s)) setSongLinks((p) => [...p, s]);
    setSongInput('');
  };
  ...
  if (error && !title) {
    return (
      <div className="max-w-xl mx-auto px-6 py-12 text-center">
        <p className="text-sm mb-4" style={{ color: '#c0524e' }}>{error}</p>
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 lg:px-10 py-8">
      ...
      <div className="flex items-center gap-2 text-sm mb-6" style={{ color: '#A9B8FF' }}>
        ...
        <span>›</span>
        <span style={{ color: '#8B97B8' }}>Edit</span>
      </div>

      <h1 className="font-heading font-semibold text-2xl mb-1" style={{ color: '#2C2C3E' }}>
        Edit Mood Space
      </h1>
      <p className="text-sm mb-8" style={{ color: '#8B97B8' }}>Update your sanctuary.</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="space-y-5">
            <div>
              ...
              <StyledTextarea value={description} onChange={setDescription}
                placeholder="Describe the feeling, scene, or memory…" rows={3} />
            </div>
            <div>
              <FieldLabel>Personal Reflection</FieldLabel>
              <StyledTextarea value={reflection} onChange={setReflection}
                placeholder="A private note about what this mood means to you…" rows={3} />
            </div>
          </div>
        </FormSection>
        ...
        <FormSection title="Emotions &amp; Tags">
          <FieldLabel>Tags <span style={{ color: '#B0BBCC' }}>(press Enter or comma)</span></FieldLabel>
          <div className="flex gap-2 mb-3">
            <input type="text" value={tagInput} onChange={(e) => setTagInput(e.target.value)}
              onKeyDown={onTagKey} placeholder="e.g. longing, amber, sunday…"
            ...
            <button type="button" onClick={addTag}
              ...
              style={{ background: 'rgba(169,184,255,0.15)', color: '#6A7FDB' }}>Add</button>
          </div>
          {moodTags.length > 0 && (
            ...
            <div>
              ...
              {coverImageUrl && (
                <div className="mt-3 h-32 rounded-xl overflow-hidden"
                  style={{ border: '1px solid rgba(169,184,255,0.2)' }}>
                  <img src={coverImageUrl} alt="Preview" className="w-full h-full object-cover" />
                </div>
              )}
            </div>
            <div>
              <FieldLabel>Color Palette <span style={{ color: '#B0BBCC' }}>(up to 6)</span></FieldLabel>
        ...
        <FormSection title="Soundtrack">
          ...
          <div className="flex gap-2 mb-3">
            <input type="url" value={songInput} onChange={(e) => setSongInput(e.target.value)}
              onKeyDown={onSongKey} placeholder="https://open.spotify.com/…"
              className="flex-1 px-4 py-2.5 rounded-xl text-sm outline-none"
              style={{ background: 'rgba(255,255,255,0.9)', border: '1.5px solid rgba(169,184,255,0.3)', color: '#2C2C3E' }}
              onFocus={(e) => { e.target.style.borderColor = '#6A7FDB'; }}
              onBlur={(e) => { e.target.style.borderColor = 'rgba(169,184,255,0.3)'; }} />
            ...
            <ul className="space-y-2">
              {songLinks.map((s, i) => (
                <li key={i} className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm"
                  ...
                  <button type="button" onClick={() => removeSong(s)} className="text-xs hover:text-red-400" style={{ color: '#B0BBCC' }}>×</button>
                </li>
              ))}
            </ul>
          )}
        ...
        <FormSection title="AI Mood Board">
          ...
          <StyledTextarea value={aiPrompt} onChange={setAiPrompt}
            placeholder="e.g. Soft autumn morning, amber light through linen curtains…" rows={3} />
        </FormSection>
